...
export default function MESHeader({ cartCount = 0 }) {
  ...
  const searchContainerRef = useRef(null); // For click outside detection
  ...
  const searchResults = React.useMemo(() => {
    ...
    searchableItems.forEach((item) => {
      ...
      } else if (descMatch || parentMatch) {
        descMatches.push(item);
      }
    });

    // Prioritize name matches, then description matches
    return [...nameMatches, ...descMatches].slice(0, 6);
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between">
            ...
            <nav className="hidden md:flex items-center gap-6" role="menubar">
              ...
              <div ref={searchContainerRef} className="relative hidden lg:block ml-6">
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
                    submitSearch();
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {item.isVariant && item.parentName && (
                            <p className="text-xs text-[#208790] truncate">
                              {item.parentName}
                            </p>
                          )}
                        </div>
          ...
          <div className="md:hidden border-t border-slate-200">
            ...
            >
              ...
              <button
                type="button"
                onClick={() => openExternal(WSP_LINK)}
                className="inline-flex items-center gap-3 rounded-xl border border-green-200 bg-green-50 px-4 py-2.5 text-sm font-medium text-green-700 hover:bg-green-100 cursor-pointer transition-colors"
              >
                <img src="/images/whatsapp-icon.png" className="h-5 w-5" alt="" />
              ...
              <button
                ...
                className={`
  inline-flex items-center gap-3 rounded-xl border px-4 py-2.5 text-sm font-medium transition-all duration-300
  ${cartPing
...
`}
              >
                ...
                Ver carrito
                {cartCount > 0 && (
                  <span className="ml-auto inline-flex h-5 min-w-[1.25rem] items-center justify-center rounded-full bg-[#208790] px-1 text-xs font-semibold text-white">
                    {cartCount}
                  </span>
                )}
              </button>
